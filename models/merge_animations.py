"""
Blender script to merge multiple animation GLB files into one.
...
# Animation files to merge (base file first - it has the mesh)
MODELS_DIR = os.path.dirname(os.path.abspath(__file__))
P1_DIR = os.path.join(MODELS_DIR, "p1")

BASE_FILE = "Meshy_AI_Animation_Idle_5_withSkin.glb"
ANIM_FILES = {
...
OUTPUT_FILE = "Character_All_Animations.glb"

print(f"Working in: {MODELS_DIR}")
print(f"P1 folder: {P1_DIR}")

# Import base file (has mesh + armature + idle animation)
base_path = os.path.join(P1_DIR, BASE_FILE)
...
if not armature:
    print("ERROR: No armature found in base file!")
    exit(1)

print(f"Found armature: {armature.name}")

# Rename the base animation to 'idle'
...
for action_name, filename in ANIM_FILES.items():
    if action_name == 'idle':
        continue  # Already have this from base

    filepath = os.path.join(P1_DIR, filename)
    ...
    # Find the newly imported armature and its action
    new_armature = None
    ...
    if new_armature and new_armature.animation_data and new_armature.animation_data.action:
        ...
        # Delete the imported armature and meshes (keep only the action)
        bpy.ops.object.select_all(action='DESELECT')
        new_armature.select_set(True)
        # Also select any meshes parented to this armature
        for obj in bpy.context.scene.objects:
...
# Create NLA tracks for each action on the main armature
...
# Add each action as an NLA track (this embeds them in the armature)
