...
- [ ] **DELETE the v1 yellow paddle flash**: the current `showOverdriveHitFX` spawns a golden sphere at the OPPONENT'S PADDLE per connected tick — confirmed confusing in playtest ("flashing in yellow makes no sense"). Remove that behavior entirely; the SIM_DEPS hook can stay wired but must no longer flash the player.
- [ ] CONNECTING end: impact blob (two spheres, white in team shell) at the GATE endpoint, scale pulsing with juiceRamp; spark bursts (existing spark/zap particle pattern) throttled ~5Hz; 2–3 vertical light pillars (thin additive planes, 0.4s life, spawn ≤1 per 0.5s).
- [ ] **DAMAGE BEATS (spec §2.3)**: presentation-side accumulator per side — while the beam connects, accumulate the opponent's observed towerHealth delta (prev-frame comparison, read-only); every ~0.6s flush the chunk as: floating damage number at the gate (reuse the existing floating-combat-text helper), health-bar pulse, impact-blob scale kick, and notify the audio layer (Task 5 boom). Reset the accumulator on block/end/interrupt. Sim untouched.
- [ ] BLOCKED end — **parry-style chromatic block (spec §2.3)**: reuse the existing parry chromatic-shake helpers (grep the parry chromatic/RGB-split effect and its `_reduceMotion` gates) to run PERSISTENTLY on the blocker's shield while the block holds, plus high-frequency positional jitter on the shield VISUAL (never the sim paddle) + perpendicular spark spray. Stop cleanly the instant the block breaks or the channel ends. Reduce Motion: steady bright shield glow instead.
